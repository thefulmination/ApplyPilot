...
def test_remaining_seconds_is_clamped_at_zero():
    budget = PhaseBudgetManager(_limits(answer=PhaseLimit(5, 1, 0.05)))
    budget.usage["answer"].elapsed_seconds = 7
    assert budget.remaining_seconds("answer") == 0
