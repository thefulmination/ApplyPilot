from applypilot.apply.launcher import _parse_terminal_result, _parse_terminal_result_detail
...
def test_placeholder_failure_reason_is_normalized_to_explicit_malformed_telemetry():
    assert _parse_terminal_result("RESULT:FAILED:reason") == "failed:unspecified"
    assert _parse_terminal_result_detail("RESULT:FAILED:reason") == "failed:unspecified"
...
def test_auth_result_detail_preserves_email_verification_reason():
    text = "RESULT:AUTH_REQUIRED:email_verification_required"
    assert _parse_terminal_result(text) == "auth_required"
    assert _parse_terminal_result_detail(text) == "auth_required:email_verification_required"
