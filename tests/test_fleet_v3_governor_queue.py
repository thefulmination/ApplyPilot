...
def test_legacy_ats_canary_exhaustion_does_not_globally_pause_linkedin(fleet_db):
    with pgqueue.connect(fleet_db) as conn:
        ...
        with conn.cursor() as cur:
            cur.execute(
                "UPDATE fleet_config SET ats_apply_mode='canary', canary_enabled=TRUE, canary_remaining=1, "
...
def test_legacy_linkedin_canary_exhaustion_does_not_block_ats(fleet_db):
    with pgqueue.connect(fleet_db) as conn:
        ...
        with conn.cursor() as cur:
            cur.execute(
                "UPDATE fleet_config SET linkedin_apply_mode='canary', linkedin_canary_enabled=TRUE, linkedin_canary_remaining=1, "
