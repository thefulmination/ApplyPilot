...
def _healthy_browser_preflight(monkeypatch):
    from applypilot.apply import browser_preflight
...
def test_make_apply_fn_retries_auth_gated_browser_failure_with_prearmed_otp(monkeypatch):
    ...
    assert out["result_metadata"]["inbox_auth"] == {
        "prearmed": True,
        "assisted_retry_count": 1,
        "hint_kind": "code",
        "parked_unsupported": False,
    }
    assert cleaned == [(3, proc)]


def test_make_apply_fn_records_magic_link_kind_without_secret(monkeypatch):
    from applypilot.apply import chrome, launcher
    from applypilot.fleet import apply_worker_main as awm

    secret_link = "https://greenhouse.io/verify?token=secret-value"
    proc = object()
    statuses = iter(("auth_required", "applied"))
    monkeypatch.setattr(chrome, "launch_chrome", lambda worker_id, **kwargs: proc)
    ...
    monkeypatch.setattr(
        launcher,
        "run_job",
        lambda *args, **kwargs: (next(statuses), 1),
    )
    monkeypatch.setattr(launcher, "_last_run_stats", {3: {}}, raising=False)
    monkeypatch.setattr(awm, "_cdp_page_urls", lambda port: [])
    ...
    assert out["run_status"] == "applied"
    assert out["result_metadata"]["inbox_auth"]["hint_kind"] == "magic_link"
...
def test_controlled_apply_successful_cleanup_is_true_and_retry_stays_bounded(monkeypatch):
    ...
    assert run_calls == [None, hint]
    assert out["assisted_retry_count"] == 1
    assert out["browser_cleanup_ok"] is True
