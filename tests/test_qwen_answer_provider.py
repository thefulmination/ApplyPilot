...
from applypilot.apply.phase_budget import PhaseBudgetManager, PhaseLimit
...
def test_local_timeout_is_clamped_to_remaining_answer_budget(monkeypatch):
    monkeypatch.setenv("APPLYPILOT_LOCAL_QWEN_ENABLED", "1")
    monkeypatch.setenv("APPLYPILOT_LOCAL_QWEN_TIMEOUT", "60")
    paid = PaidClient()
    seen = {}

    def transport(_payload, timeout):
        seen["timeout"] = timeout
        return {"message": {"content": GOOD}}

    budget = PhaseBudgetManager({"answer": PhaseLimit(5, 1, 0.05)})
