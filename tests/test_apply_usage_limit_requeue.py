...
def _patch_launcher_agent_io(monkeypatch, tmp_path, *, stdout_text="", popen_error=None):
    ...
    monkeypatch.setattr(
        ...
        lambda proc: type("Guard", (), {"release": lambda self: None})(),
    )
    return state
...
def test_apply_fn_forwards_adapter_metadata_from_launcher_stats(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda _port: {"ready": True, "reason": "ready"},
    )
    monkeypatch.setattr(browser_preflight, "clear_readiness_cache", lambda _port: None)
    monkeypatch.setattr(
        launcher,
        "run_job",
