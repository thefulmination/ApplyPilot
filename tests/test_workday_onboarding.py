...
class Locator:
    ...
    def evaluate(self, _js):
        if "click" in _js: self.page.clicked.append(self.selector)
        return True
    ...
    def click(self, **_kwargs):
        ...
        if "Checkbox" in self.selector: self.page.checked = True
...
class Page:
    def __init__(self, *, ready=False, captcha=False):
        ...
        if ready: self.visible.add('input[type="file"]')
        if captcha: self.visible.add('iframe[src*="recaptcha" i], iframe[src*="hcaptcha" i], .g-recaptcha, .h-captcha')
