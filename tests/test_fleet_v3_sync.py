...
def enable_supported_ats_adapters(monkeypatch):
    ...
    with pgqueue.connect(fleet_db) as pg:
        ...
        with pg.cursor() as cur:
            cur.execute("SELECT url, target_host, apply_domain, dedup_key, approved_batch, "
                        ...
                        "eligibility_status, eligibility_reason, liveness_status, "
                        "liveness_reason, liveness_checked_at FROM apply_queue")
            rows = cur.fetchall()
