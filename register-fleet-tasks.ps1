...
#     home only    : Watchdog, Doctor, OutcomeScan, DiscoveryIngest, OutcomesScan (daily 07:00)
...
function Get-TaskNamesForMachine([string]$m) {
  ...
  if ($m -eq "home") { $names += @("${TaskPrefix}Watchdog", "${TaskPrefix}Doctor", "${TaskPrefix}OutcomeScan", "${TaskPrefix}DiscoveryIngest", "${TaskPrefix}OutcomesScan") }
...
"@
  ...
  # -- OutcomesScan (spec 2026-07-03-outcome-loop-integrity-design.md, Amendments) -------------
  # Daily 07:00 pass of `run-applypilot.ps1 outcomes-scan` -- the email_events WRITER (distinct
  # from the OutcomeScan block above, which chains outcomes-scan + reconcile-email every 6h via
  # the raw .exe). This task invokes the wrapper script directly so it inherits run-applypilot's
  # live-brain DB pinning (APPLYPILOT_DB_PATH -> %LOCALAPPDATA%\ApplyPilot\applypilot.db) and its
  # post-write local versioned backup. Kept as a SEPARATE named task (not folded into OutcomeScan)
  # so the two schedules -- 6h chain vs. daily 07:00 -- can be reasoned about and unregistered
  # independently.
  Write-Host "`n[register-fleet-tasks] registering OutcomesScan (daily 07:00, run-applypilot.ps1 outcomes-scan)..." -ForegroundColor Cyan
  ...
  $outcomesScanLog = Join-Path $logDir "outcomes-scan-daily.log"
  $outcomesScanWrapperContent = @"
# Auto-generated by register-fleet-tasks.ps1 -- do not hand-edit; re-run the register script instead.
# Daily email_events writer. run-applypilot.ps1 pins APPLYPILOT_DB_PATH to the live brain and
# backs up the DB after write commands -- no env setup needed here beyond invoking it.
`$ErrorActionPreference = 'Continue'
Set-Location '$repo'
& '$runApplypilotPs1' outcomes-scan *>> '$outcomesScanLog'
exit `$LASTEXITCODE
"@
  $outcomesScanWrapper = Write-Wrapper "outcomes-scan-daily-task" $outcomesScanWrapperContent
  $outcomesScanTrigger = New-ScheduledTaskTrigger -Daily -At 7:00am
  $outcomesScanSettings = New-ScheduledTaskSettingsSet -StartWhenAvailable -ExecutionTimeLimit (New-TimeSpan -Hours 2)
  Register-FleetTask -Name "${TaskPrefix}OutcomesScan" -WrapperPath $outcomesScanWrapper -Trigger $outcomesScanTrigger -Settings $outcomesScanSettings `
    -Description "ApplyPilot daily outcomes scan: run-applypilot.ps1 outcomes-scan at 07:00, writes email_events off the live brain."
}
