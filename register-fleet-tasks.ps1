...
#     home only    : Watchdog, Doctor, Autotriage, OutcomeScan, DiscoveryIngest
...
function Get-TaskNamesForMachine([string]$m) {
  ...
  if ($m -eq "home") { $names += @("${TaskPrefix}Watchdog", "${TaskPrefix}Doctor", "${TaskPrefix}Autotriage", "${TaskPrefix}OutcomeScan", "${TaskPrefix}DiscoveryIngest", "${TaskPrefix}ComputeIngest") }
