...
#     home only    : Watchdog, OtpResponder, Doctor, Autotriage, OutcomeScan, DiscoveryIngest
...
function Get-TaskNamesForMachine([string]$m) {
  ...
  if ($m -eq "home") { $names += @("${TaskPrefix}Watchdog", "${TaskPrefix}OtpResponder", "${TaskPrefix}Doctor", "${TaskPrefix}Autotriage", "${TaskPrefix}OutcomeScan", "${TaskPrefix}DiscoveryIngest", "${TaskPrefix}ComputeIngest") }
...
"@
  ...
  $otpExe = $null
  foreach ($d in @(".\.conda-env\Scripts", ".\.venv\Scripts")) {
    $cand = Join-Path $repo (Join-Path $d "applypilot-fleet-otp-home.exe")
    if (Test-Path $cand) { $otpExe = $cand; break }
  }
  if (-not $otpExe) {
    Write-Host "  SKIPPED: applypilot-fleet-otp-home.exe not found in .conda-env or .venv (run 'pip install -e .' first)." -ForegroundColor Yellow
  } else {
    $otpLog = Join-Path $logDir "otp-responder.log"
    $otpWrapperContent = @"
# Auto-generated by register-fleet-tasks.ps1 -- do not hand-edit; re-run the register script instead.
...
function Stop-StaleOtpResponderProcesses {
  ...
  Get-CimInstance Win32_Process -ErrorAction SilentlyContinue |
    Where-Object {
      `$_.ProcessId -ne `$self -and `$_.CommandLine -and
      `$_.CommandLine -like "*`$repoRoot*" -and
      (`$_.CommandLine -like '*applypilot-fleet-otp-home.exe*' -or `$_.CommandLine -like '*otp-responder-task.ps1*')
    } |
    ForEach-Object {
      Stop-Process -Id `$_.ProcessId -Force -ErrorAction SilentlyContinue
    }
...
& '$otpExe' --dsn `$env:FLEET_PG_DSN *>> '$otpLog'
"@
...
`$env:FLEET_PG_DSN = '$effectiveDsn'
Set-Location '$repo'
& '$autotriagePs1' -Once -Limit 25 -WindowMinutes 1440 *>> '$autotriageLog'
"@
