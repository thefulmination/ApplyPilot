...
**Non-goals (Sub-project A):** the LinkedIn lane (`_tick_linkedin`, `lease_linkedin`, the single-event halt) — deferred to **Sub-project B**, built after A is proven; the broker/friend-machine RPC; resume tailoring (apply as-is); a **visual UI** — deferred to a dedicated **unified fleet-console sub-project** after A is proven (one local dashboard over all lanes: watchdog `dashboard_snapshot`, apply queue + canary/approve controls, the captcha inbox, spend). A ships CLI-operable (incl. the `resolve-challenge` captcha-inbox command, §3.2) so the console is ergonomics, not a functional gate. Also out of A: the fuzzy near-duplicate Jaccard port (an exact `company|title` push exclusion is the A floor; fuzzy port is a follow-up).
...
- `challenges` — list open (`resolved_at IS NULL`) `auth_challenge` rows (the captcha inbox the fleet parked).
- `resolve-challenge <url> [--skip]` — thin wrapper over the existing `queue.resolve_challenge(conn, url, *, requeue=…)` (verified built but with NO runtime driver today). Default releases the parked job back to `status='queued'` (re-leasable) and resolves the `auth_challenge` row; `--skip` closes it to `status='blocked'`. This closes the one real control gap: today a fleet-parked captcha has no owner path to release.
- `status` — queue depth by status + `paused`/`canary_remaining`/`spend_cap_usd` vs spend, open-challenge count, and applied/parked/crash counts, so the owner sees attempts-vs-applies.
...
- **Enforced at the lease (hard) AND the worker (belt), keyed off the same locked `fleet_config` row:** (a) `_LEASE_APPLY` (which already locks `fleet_config FOR UPDATE` for the canary) gains, in the same `cfg` CTE, both `NOT COALESCE(cfg.paused, FALSE)` (paused guard) AND `COALESCE(cfg.spend_cap_usd,0) <= 0 OR (SELECT COALESCE(SUM(est_cost_usd),0) FROM apply_queue) < cfg.spend_cap_usd` (the **HARD cost-cap guard** — a paused/over-cap fleet serves no apply lease, atomically, like the canary); (b) the worker loop also calls `should_halt(conn)` before each lease and idles when true (the soft belt — and the path that lets the watchdog's `paused` flip stop an in-flight worker). `paused` is `NOT NULL DEFAULT FALSE`, so the guards are two-valued and transparent for un-armed/un-capped rows. The canary auto-pause (G3), the cost cap (G5), the watchdog cap-pause, and `should_halt` all converge on the one locked `fleet_config` row.
...
- A ships CLI-operable incl. a new `resolve-challenge` command (closes the parked-captcha control gap); a visual operator console is deferred to a dedicated **unified fleet-console sub-project** after A is proven. **Decided (owner).**
- Sub-project B (LinkedIn lane) is a separate spec after A is proven. **Decided (owner).**

**Downstream sub-projects (sequenced, not in A):** B — fleet LinkedIn lane; C — unified visual fleet console (all-lane dashboard + captcha inbox + controls).
