...
#                                  3. applypilot-fleet-apply-home approve --all-pushed
#                                  4. applypilot-fleet-apply-home lift-canary
...
# ---------------------------------------------------------------------------------------------
# ApplyCycle -- every 4h. verify-live (fresh pre-push check) -> push -> approve --all-pushed ->
# lift-canary -> resume-if-safe -> pull. Each step logged; wrapper exits non-zero if any fails.
# ---------------------------------------------------------------------------------------------
Write-Host "`n[register-apply-cycle] registering ApplyCycle (every 4h: verify-live -> push -> approve -> lift-canary -> resume-if-safe -> pull)..." -ForegroundColor Cyan
