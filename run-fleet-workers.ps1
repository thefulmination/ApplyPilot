# run-fleet-workers.ps1 -Count N [-Agent claude|codex] [-Label home] [-Model name]
...
#     .\run-fleet-workers.ps1 -Count 4 -Agent codex -Label m2   # 4 Codex workers on machine 2
#
...
$existing = Get-CimInstance Win32_Process -Filter "Name='applypilot-fleet-apply.exe' OR Name='python.exe'" -ErrorAction SilentlyContinue |
    Where-Object { $_.CommandLine -and $_.CommandLine -match 'fleet-apply|apply_worker_main' }
if ($existing) {
  Write-Host ("Stopping {0} existing apply worker process(es) for a clean slate..." -f @($existing).Count) -ForegroundColor Yellow
...
Write-Host ("`n{0} isolated workers up (slots 0..{1}), each its own Chrome. Watch them at the console: http://<your-lan-ip>:8787" -f $Count, ($Count - 1)) -ForegroundColor Cyan
