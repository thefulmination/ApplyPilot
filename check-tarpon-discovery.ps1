    ...
    with psycopg.connect(dsn, autocommit=True, row_factory=dict_row) as conn:
        ...
        cur.execute("""
          SELECT worker_id, machine_owner, home_ip, role, state, current_job,
                 ...
                 left(coalesce(recent_log,''), 1000) AS recent_log,
...
if (-not $mainDisc) {
  $issues.Add("No discovery heartbeat row found for the required m2-disc worker.")
} elseif ([int]$mainDisc.age_s -gt $AliveSeconds) {
  $issues.Add("Discovery heartbeat is stale: $($mainDisc.age_s)s > ${AliveSeconds}s.")
}

# A single healthy heartbeat is insufficient: the worker set can be partially
# dead, version-skewed, or repeatedly recovering from a database deadlock.
$expectedWorkers = @("m2-disc") + (0..7 | ForEach-Object { "m2-disc-$($_)" })
$tarponWorkers = @($data.discovery_workers | Where-Object {
  $_.worker_id -eq "m2-disc" -or $_.worker_id -like "m2-disc-*"
})
$observedWorkers = @($tarponWorkers | ForEach-Object { $_.worker_id })
foreach ($workerId in $expectedWorkers) {
  if ($workerId -notin $observedWorkers) {
    $issues.Add("Missing discovery heartbeat row: $workerId.")
  }
}
foreach ($worker in $tarponWorkers) {
  if ([int]$worker.age_s -gt $AliveSeconds) {
    $issues.Add("Discovery worker heartbeat is stale: $($worker.worker_id) age=$($worker.age_s)s.")
  }
  if ($worker.state -eq "version_mismatch") {
    $issues.Add("Discovery worker version mismatch: $($worker.worker_id) sw=$($worker.sw_version).")
  }
  if ($worker.last_error -match "DeadlockDetected|deadlock detected" -and
      $worker.recent_log -notmatch "wrote search") {
    $warnings.Add("Discovery worker's latest recorded error is a PostgreSQL deadlock: $($worker.worker_id).")
  }
}
$workerVersions = @($tarponWorkers | Where-Object { $_.sw_version } | Select-Object -ExpandProperty sw_version -Unique)
if ($workerVersions.Count -gt 1) {
  $issues.Add("Discovery worker versions differ: $($workerVersions -join ', ').")
}
