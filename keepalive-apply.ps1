...
$hardFaultDir = "C:\Users\JStal\AppData\Local\ApplyPilot\lifecycle-faults"
...
$hardFaultFiles = @()
if (Test-Path -LiteralPath $hardFaultMarker -PathType Leaf) {
    $hardFaultFiles += Get-Item -LiteralPath $hardFaultMarker
}
if (Test-Path -LiteralPath $hardFaultDir -PathType Container) {
    $hardFaultFiles += Get-ChildItem -LiteralPath $hardFaultDir -Filter "fault-*.json" -File
}
if ($hardFaultFiles.Count -gt 0) {
    if ($env:APPLYPILOT_RECONCILE_HARD_FAULT -ne "1") {
        Log "HARD-FAULT record(s) present -- refusing supervisor relaunch"
        exit 3
    }
    Log "HARD-FAULT record(s) present -- explicit reconciliation requested"
}
...
& $py -m applypilot.cli supervise-apply --target-applied $TargetApplied --max-cost-usd 90 `
    --workers 1 --linkedin-daily-cap 0 --max-job-age-days 45 --stall-minutes 20 `
