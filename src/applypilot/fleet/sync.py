...
SELECT url, company, title, application_url, {location_expr}, full_description,
       ...
       liveness_status, {liveness_reason_expr}, {liveness_checked_expr}
FROM jobs
    ...
    *,
    ...
    try:
        ...
        base_sql = _PUSH_APPLY_SELECT.format(
            ...
            liveness_reason_expr=(
                "liveness_reason" if _jobs_column_exists(sq, "liveness_reason")
                else "NULL AS liveness_reason"
            ),
            liveness_checked_expr=(
                "last_verified_live" if _jobs_column_exists(sq, "last_verified_live")
                else "NULL AS last_verified_live"
            ),
            company_blocklist=company_blocklist,
            thin_description_chars=THIN_DESCRIPTION_CHARS,
            attempts_predicate=(
                f"COALESCE(apply_attempts, 0) < {int(config.DEFAULTS['max_apply_attempts'])}"
                if _jobs_column_exists(sq, "apply_attempts")
                else "1=1"
            ),
        )
