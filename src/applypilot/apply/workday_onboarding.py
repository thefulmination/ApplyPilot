...
def _driver_error_reason(prefix: str, exc: Exception, *, secrets=()) -> str:
    """Keep browser diagnostics actionable while redacting credential values."""
    detail = str(exc).splitlines()[0].strip()[:180]
...
def _sign_in(page, *, email: str, password: str) -> WorkdayOnboardingResult:
    ...
    try:
        ...
        return WorkdayOnboardingResult(
            "parked",
            _driver_error_reason("signin_driver_error", exc, secrets=(email, password)),
        )
...
def _reset_password(page, *, email: str, password: str, host: str,
    ...
    try:
        ...
        return WorkdayOnboardingResult(
            "parked",
            _driver_error_reason("reset_request_error", exc, secrets=(email, password)),
        )
    if watch_fn is None:
    ...
    try:
        ...
        return WorkdayOnboardingResult(
            "parked",
            _driver_error_reason("password_reset_error", exc, secrets=(email, password)),
        )
    if _application_step_visible(page):
...
def bootstrap_workday_account(page, *, email: str, password: str,
    ...
    try:
        ...
        return WorkdayOnboardingResult(
            "parked",
            _driver_error_reason(
                "create_account_driver_error", exc, secrets=(email, password)
            ),
        )
    if _captcha_present(page):
        return WorkdayOnboardingResult("captcha", "workday_create_account_captcha")
